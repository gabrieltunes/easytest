			...
			<div class="card-body">
				...
				@endif
	    			...
	    			<div class="form-group row">
			    		<label for="materia_id" class="col-sm-3 col-form-label font-weight-bold control-label">{{ __('Disciplina:') }}</label>
					    <div class="col-sm-8">
					      <select name="materia_id" class="form-control" name="materia_id">
                            @foreach ($materias as $materia_id)
                                <option value="{{$materia_id->id}}" @if($assunto->materia_id==$materia_id->id) selected @endif > {{$materia_id->descricao}}</option>
                            @endforeach     
                          </select>
					    </div>
			  		</div>
